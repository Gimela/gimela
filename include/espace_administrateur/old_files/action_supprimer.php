...
elseif($_SESSION['id_statut'] >= STATUT_ACCES_ADMINISTRATION)
	...
	if($_GET['id']){
		...
		echo('<!DOCTYPE html>
			<html lang="fr">
				<head>
					<meta charset="UTF-8">
					<title>Interface membre du CVVFR</title>
				</head>
				<body>
				<h1> Membre  </h1>
	...
	if(isset($_POST["delete"])){
		...
		$fileName=$resultat1["id_club"]."-".$resultat1["nom"].".csv";
		$fp=fopen($fileName,"w+");
		fputs($fp,$row);
		fputcsv($fp,$resultat1,";");
		fclose($fp);
		...
		rename($fileName,$dossier.$fileName);
		
		if(isset($fileName)){
			if($reqSuppresion->execute(array(':id'=>$_GET["id"]))){
			echo'<br> membre supprimer ';
